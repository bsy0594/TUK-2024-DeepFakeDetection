...
def run_inference(
    ...
    # frame_dir = script_dir / "temp_frames_infer"

    print("▶ Extracting frames…")
    n_frames = extract_frames(str(video_path), str(frame_dir), interval=1)
    print(f"✔ Extracted {n_frames} frames to '{frame_dir}'\n")

    # 4) DataLoader 준비
    _, val_tf = get_ffpp_transforms(cfg["input_size"])
    frame_files = sorted(map(str, frame_dir.glob("*.jpg")))
