...
class FFPPFrameDataset(Dataset):
    ...
    def frame_paths(self) -> List[str]:
        """
        FFPPFrameDataset.image_paths 리스트를 문자열 경로 리스트로 반환합니다.
        """
        # Path 객체가 아니라 string 을 원하면 str(p) 로 바꿔도 좋습니다.
        return [str(p) for p in self.image_paths]

    @property
    def video_ids(self) -> List[str]:
        """
        각 이미지의 부모 폴더명을 video ID로 반환합니다.
        """
        return [Path(p).parent.name for p in self.frame_paths]